...
int main(int argc, char *argv[])
{
   ...
   {
      slave = atoi(argv[2]);
      if ((strncmp(argv[3], "-i", sizeof("-i")) == 0))
         mode = MODE_INFO;
      if (argc > 4)
      {
         if ((strncmp(argv[3], "-r", sizeof("-r")) == 0))
         {
            mode = MODE_READBIN;
            printf("read mode selected");
         }
         if ((strncmp(argv[3], "-ri", sizeof("-ri")) == 0))
            mode = MODE_READINTEL;
         if ((strncmp(argv[3], "-w", sizeof("-w")) == 0))
            mode = MODE_WRITEBIN;
         if ((strncmp(argv[3], "-wi", sizeof("-wi")) == 0))
            mode = MODE_WRITEINTEL;
